...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 scales=4, k=4, last_height=16, last_width=16,
    ...
    def forward(self, src: List[Tensor],
        ...
        query_ref_point = self.query_ref_point_proj(tgt.unsqueeze(dim=2))
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        for src, ref_point, src_mask, src_key_padding_mask, pos in zip(src_tensors,
                                                                       ref_points,
                                                                       src_masks,
                                                                       src_key_padding_masks,
                                                                       poses):
            # src = self.with_pos_embed(src, pos)
            src2, attns = self.ms_deformbale_attn(src, src_tensors, ref_point)
    ...
    def forward_pre(self,
        ...
        for src, ref_point, src_mask, src_key_padding_mask, pos in zip(src_tensors,
            ...
            src2, attns = self.ms_deformbale_attn(src2, src_tensors, ref_point)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        # B, L, 1, C
        tgt2, attns = self.ms_deformbale_attn(tgt.unsqueeze(dim=2), memory, ref_point)
    ...
    def forward_pre(self, tgt: Tensor,
        ...
        memory = [self.with_pos_embed(tensor, pos) for tensor, pos in zip(memory, poses)]
        # B, L, 1, 2
        tgt2, attns = self.ms_deformbale_attn(tgt2.unsqueeze(dim=2), memory, ref_point)
        if self.need_attn:
            self.attns.append(attns)
        tgt2 = tgt2.squeeze(dim=2)
